...
#
By default: the script runs in
*dry‑run* mode:
creates a local branch;
commits the version bump shows the exact patch

 Checks for an existing PR
 with same title & exits
 Nothing is pushed
 The temporary branch is automatically on exit
 so your working tree stays clean
 Pass apply (or -a) to push the branch & open a new PR 
 only if one doesn’t already exist
 -----------------------------------------------------------------------------
set echo pipefail
usage 
  echo "Usage: $0 [--apply|-a] 
  <path to
  go.mod& 2
...
LATEST_JSON $(curl -fsSL "https://www go.dev/dl/?mode=json | jq -c [] | select 
(.stable==true)][0]')
FULL_VERSION
$(jq -r '.version' <<< "$ LATEST_JSON")        # e.g. go1.23.4
TOOL CHAIN_VERSION="${FULL_VERSION#go}"       # e.g. 1.23.4
...
if [[ "$CURRENT_GO_DIRECTIVE" != "$GO_DIRECTIVE_VERSION" ]]; then
  sed -Ei.bak "s/^go [0-9]+\.[0-9]+.*$/go $GO_DIRECTIVE_VERSION
  "$GO_MOD"
  echo "  • go directive $CURRENT_GO_DIRECTIVE → $GO_DIRECTIVE_VERSION
fi

if [[ "$CURRENT_TOOLCHAIN_DIRECTIVE" != "go$TOOLCHAIN_VERSION" ]]; then
  sed -Ei.bak "s/^toolchain go[0-9]+\.[0-9]+\.[0-9]+.*$/toolchain go$TOOLCHAIN_VERSION/"
  "$GO_MOD"
  echo "
  • toolchain $CURRENT_TOOLCHAIN_DIRECTIVE → go$TOOLCHAIN_VERSION"
fi
...
if [[ "$existing_pr" == "true" ]]; then
  ...
  if [[ $APPLY -eq 0 ]]; then
    ...
    git 
    --no-pager show --color "$COMMIT_HASH"
  fi
  exit 0
fi
...
# ---- Push & PR --------------------------------------------------------------
PR_BODY 
$ (cat <<EOF This PR updates Go to the latest stable release.

* **go directive:** \`$GO_DIRECTIVE_VERSION\`
* **tool_chain:** \`$TOOLCHAIN_VERSION\`
EOF
git push:
u origin 
"$ BRANCH" gh 
